...
public class CommonUtils {
	...
	public static void readPropertiesFile() throws Exception {
		...
		try {
			...
			fis = new FileInputStream("C:\\Users\\Hp\\Downloads\\srujana1.png");

			// create the memory space for that config.properties file
...
//The method launches a specified web browser and navigates to a URL.	
	public static void launchBrowserAndNavigateToApp() {
		...
		try {
			...
			String broswerName =  config.getProperty("browser");//firefox
...
//The method switches the browser's focus to a specified iframe.
	...
	// it clears any existing text from the web element before entering the new
	// value. This method enters the value into the input field.
	...
	// The method is used to delete all the text in a web element (like a text box
	// or input field).
...
//method is used to switch the browser's focus from the current window to a new one (usually a popup or a new tab).
	...
	public static void hardWait(int millis) {
		try {
			Thread.sleep(millis * 1000);// wait 5 sec
		} catch (Exception e) {
			TestNGUtility.assertFail(e.getMessage());
		}
	}
	...
	// method checks if a web element is selected or checked (typically used for
...
//to wait for a specific web element to become visible on a web page
	...
	// This method navigates back to the previous page in the browser's history.
	public void goBack() {
		try {
	...
	// This method navigates forward to the next page in the browser's history, if
	// available.
